...
必须特别区分“轴上只有一个孔”和“任意注视角度都只有一个孔”。当前扫描图中的有效孔数是轴上静态值。对于偏离中心的角度 $\theta$，孔径在瞳孔面上的有效位置要写成 $r_n-z\tan\theta$。同样一个孔在某个视角下可能退出瞳孔，另一个孔则可能进入。多孔设计能否覆盖视野，正是由这些随视角移动的接受区间决定的。详细极限见第 4.3 节。
...
| 孔距 `p` / mm | `p/D` | 轴上有效孔 `N_eff` | 一阶角 / arcmin | 最大重影峰比 | 非中央孔能量占比 | 主要现象 |
|---:|---:|---:|---:|---:|---:|---|
...
| `4.0` | `0.75` | `1 / 81` | `41.3` | `0.00` | `0.00` | 轴上只剩中央孔，大角度仍可能与邻孔双覆盖 |
| `8.0` | `1.51` | `1 / 81` | `82.5` | `0.00` | `0.00` | 轴上退化为单孔，但孔间会出现视角覆盖空隙 |

表中的 `N_eff` 是轴上静态有效孔数，不代表任意注视角度下的孔数。`p` 增大确实把重影中心间距按 $M p$ 拉开，但它同时迅速减少与瞳孔交叠的孔数。对给定 $M$ 和 $d$，重影最强不一定在最大孔距，而可能在“仍有两个以上孔进入瞳孔、且每个孔仍有足量通光”的中间区域。各 E 视标面板中的 `shift/W` 是 $M p$ 除以 E 视标宽度，`overlap = 1 - shift/W` 是图像层面的重叠程度；`Md/W` 是单孔离焦模糊相对视标宽度。`overlap` 接近 `1` 表示多个错位像大面积叠在一起，接近 `0` 才接近彼此分离的像。图注中采用 `p/D` 而不是 $p/f$，因为它直接回答轴上观察时瞳孔里同时能看到几个孔。

#### 4.3 单孔退化极限、无盲区与无重影的不可兼得

这里必须先区分三个不同问题：

1. **轴上单孔**：正对中心孔时，相邻孔是否进入瞳孔。
2. **全视场无盲区**：视线移到任意方向时，是否至少有一个孔能透光。
3. **全视场无重影**：任意注视方向是否最多只有一个孔能同时透光。

第 1 个问题的答案可以很小，但把第 1 个答案当成第 2、3 个问题的答案是不对的。

设瞳孔半径为 $D/2$、孔半径为 $d/2$，一个孔心相对瞳孔中心偏移 $r$。孔圆与瞳孔圆存在正面积交叠的最大偏移为：

$$
b=\frac{D+d}{2}.
$$

$r=b$ 时两圆相切，交叠面积只有零；因此严格的有效孔应使用 $r<b$。在文献参数 `d = 0.9 mm`、瞳孔 `D = 5.3 mm` 下，
...
轴上注视时，中心孔位于 $r=0$。相邻孔位于 $r=p$。要让相邻孔在轴上不进入瞳孔，需要：

$$
p\ge p_{\mathrm{axis}}=\frac{D+d}{2}=b.
$$

对文献参数 `d = 0.9 mm`、`D = 5.3 mm`：

$$
p_{\mathrm{axis}}=3.10\ \mathrm{mm}.
$$

所以仿真中 `p = 4 mm` 和 `8 mm` 在轴上只剩中心孔，这是正确结果。

**全视场最多单孔极限**

当视线偏离光轴 $\theta$ 时，孔心在瞳孔面上的有效位置变为：
...
$$
r_n-b<z\tan\theta<r_n+b.
$$

把 $u=z\tan\theta$ 看成视角平面的坐标，每个孔贡献一个半径为 $b$、圆心为 $r_n$ 的圆形接受区。若相邻孔心距为 $p$，则：

- 若 $p<2b$，存在视角区间同时落入两个或更多接受区，会产生双覆盖和潜在重影。
- 若 $p\ge 2b$，没有两个孔的接受区发生正面积重叠。
- 但 $p\ge 2b$ 只说明“最多一个孔”，并不保证“至少一个孔”。圆与圆之间仍会留下没有孔覆盖的盲区。

因此，全视场“最多一个孔”的严格极限是：

$$
p_{\mathrm{single}}\ge 2b=D+d.
$$

对 `d = 0.9 mm`、`D = 5.3 mm`：

$$
p_{\mathrm{single}}=6.20\ \mathrm{mm}.
$$

**全视场无盲区极限**

对正方形孔阵，一个正方形原胞中离最近孔心的最大距离为 $p/\sqrt 2$。要让每个视角都被至少一个接受圆覆盖，需要：

$$
p\le p_{\mathrm{cover,square}}=\sqrt{2}b
=\frac{D+d}{\sqrt 2}
=4.38\ \mathrm{mm}.
$$

对三角形六近邻晶格，覆盖半径是 $p/\sqrt 3$，所以：

$$
p\le p_{\mathrm{cover,triangular}}=\sqrt{3}b
=5.37\ \mathrm{mm}.
$$

注意覆盖极限是**上界**，无重叠极限是**下界**。当前参数得到：

$$
\begin{array}{c|c|c}
\text{阵列} & \text{无盲区上限} & \text{无双覆盖下限}\\
\hline
\text{正方形} & 4.38\ \mathrm{mm} & 6.20\ \mathrm{mm}\\
\text{三角形} & 5.37\ \mathrm{mm} & 6.20\ \mathrm{mm}
\end{array}
$$

两种阵列都满足“无盲区上限 < 无双覆盖下限”。因此，在同一平面、同一孔径、相同圆形瞳孔的模型里，不可能同时做到任意视角无盲区和任意视角至多一个孔。多孔的作用本来是给各个注视方向保留至少一个可透光孔；若要把这些接受区连成无空档的覆盖，相邻区域就需要发生一定重叠，也就保留了产生重影的几何通道。

程序对正方形阵列的一个原胞做了数值覆盖积分，结果如下：

| 孔距 `p` / mm | `0` 个孔占比 | 恰好 `1` 个孔占比 | 至少 `2` 个孔占比 | 含义 |
|---:|---:|---:|---:|---|
| `3.10` | `0` | `0` | `1.000` | 只有正中心是严格单孔；任意有面积的偏轴方向都发生多孔重叠 |
| `4.00` | `0` | `0.229` | `0.771` | 全视场无盲区，但约 `77.1%` 的方向有多孔重叠 |
| `6.20` | `0.215` | `0.785` | `0` | 不再发生双覆盖，但约 `21.5%` 的方向没有孔可用 |
| `8.00` | `0.528` | `0.472` | `0` | 约 `52.8%` 的方向是无光盲区 |

这里得到的矛盾是：`p = 3.10 mm` 是轴上单孔极限，不等于整片视野的单孔极限；`p = 4 mm` 为了解决视角盲区，让约四分之三的观察方向带上了多孔重叠；`p = 8 mm` 虽然不会双覆盖，却让超过一半的视角看不到孔。于是“为了避免重影就一路增大孔距，最终退化为单孔”并不是完整设计逻辑，因为它同时把可观察方向压缩成了离散窗口。

工程上可以设置一个孔权阈值 $\eta$，把交叠面积占比低于 $\eta$ 的孔视为无效，得到有效半径 $b_\eta$，再把上面的 $b$ 替换为 $b_\eta$。但这只改变数值边界，不改变二维等圆接受区的拓扑结论。若还要同时保留连续视野和单孔成像，需要使用角选择结构，例如带遮挡壁的微通道、双层错位孔阵、非周期排布或随视角改变透光的结构；仅靠调整同一平面上的单一孔距不能完成这件事。

$$
p_{\mathrm{axis},\eta}=b_\eta,
\qquad
p_{\mathrm{single},\eta}=2b_\eta,
\qquad
p_{\mathrm{cover,square},\eta}=\sqrt{2}b_\eta.
$$

这里的覆盖图是几何上界和设计筛选图，使用圆孔圆瞳、$z\tan\theta$ 平移、正面积交叠和忽略孔壁厚度的一阶模型。它不替代真实瞳孔倾斜、镜片顶点距离、眼动、像差、衍射和主观重影阈值的完整光线追踪。

![单孔退化极限、双覆盖与视角盲区](output/pinhole_array_ghost/view_angle_coverage.png)
...
4. `p/D` 决定轴上瞳孔能否同时接收多个孔。$p/D$ 超过约 `0.58` 后，文献参数下的边孔在轴向上开始退出瞳孔；但偏心视角会重新把邻孔移入接受区。即使某个视角只剩一个孔，也不能据此推断整个视场都没有双覆盖。
...
3. `p/D` 和实际孔径交叠决定瞳孔中有多少孔参与成像。孔距过小，多个像强烈重叠并可能发糊；孔距超过 `3.10 mm` 后，轴上会退化为单孔。但全视场最多单孔要求 `p >= 6.20 mm`，而这个孔距又会留下盲区。多孔阵列无法同时满足全视场无盲区和任意视角至多一个孔。
...
仍需保留的限制是：主扫描中的瞳孔权重采用轴上静态圆交叠，即只按孔圆与瞳孔圆的交叠面积筛选孔，尚未在每一张 E 视标阵列图中逐视角使用动态条件 $|r_n-z\tan\theta|<D/2+d/2$。新增的视角覆盖图单独计算了这项几何边界，用来确定单孔退化、双覆盖和盲区，但没有重新卷积整套 E 视标。对约 `±20 arcmin`、镜片到瞳孔距离约 `12 mm` 的主扫描视场，横向项约为 `0.07 mm`，远小于毫米级瞳孔和孔距，因此当前近似不会改变本阶段主线；更大偏心视角仍需使用动态孔权。本阶段也没有加入真实绝对照度、镜片透过率、光子噪声、视锥采样、眼内色差和高阶像差。全图阵列卷积仍假设 PSF 在单个 E 视标范围内空间不变；太阳盘按面积加权方向积分，尚未模拟云层、瞬态眼动和观察者对重影方向的适应性。
...
2. **瞳孔有效孔数**：一个孔是否能参与成像不能只比较孔心与瞳孔中心，还要比较孔径圆和瞳孔圆的交叠面积。瞳孔缩小或孔距增大时，轴上有效孔数减少；但“某个视角只剩一个孔”和“全视场最多一个孔”是两个不同结论。
...
5. **覆盖边界**：文献参数下轴上单孔极限为 `3.10 mm`，全视场无双覆盖下限为 `6.20 mm`，正方形阵列无盲区上限为 `4.38 mm`，三角形阵列无盲区上限为 `5.37 mm`。覆盖上限低于无双覆盖下限，所以一维口径不能代替二维视野设计。

#### 11.2 已经由当前参数扫描支持

1. 在 `M = 3 D`、`p = 2 mm` 时，相邻子像中心只错开约 `20.6 arcmin`，而 E 视标宽 `100 arcmin`，因此最典型的结果是主像附近大面积重叠，而不是很多完整小 E。
2. 在 `d = 0.9 mm`、`D = 5.3 mm` 的文献参数附近，孔距从 `2 mm` 增大到 `4 mm` 时，理论角间距持续增大，但轴上有效孔数从 `9` 降到 `1`；重影强度不是孔距的单调函数。`p = 4 mm` 的二维覆盖积分仍有约 `77.1%` 的方向同时通过至少两个孔，说明轴上单孔不能消除偏轴重影。
...
在不受主观判断影响的情况下，验证以下五条关系：

1. 重影角位置是否严格按 `Delta theta = M p` 变化。
2. 有限瞳孔下有效孔数是否由孔径圆与瞳孔圆的交叠决定，并区分轴上单孔与偏轴重新进入瞳孔的邻孔。
3. 单个重影的几何宽度是否趋近 `M d`，并在小孔径时逐渐受衍射限制。
4. 太阳大小扩展源经过阵列 PSF 后，峰间谷值是否符合仿真预测。
5. 改变视角时，零孔、单孔和多孔区域是否符合 `r_n-z tan(theta)` 的覆盖边界。
...
5. 对正方形孔板，视角平移序列应覆盖多孔区、无盲区多孔重叠区、同时有盲区和重叠区，以及无重叠但有空隙区，并检查 `4.38 mm` 与 `6.20 mm` 两组边界。
...
1. 轴上单孔区域：中心视线只剩一个孔，但亮度和偏轴视野可能变差。
2. 强重叠区域：多个孔有足够通光，但主像发粗，可能出现双影或三影。
3. 有限分离区域：多个峰可以被部分识别，是研究重影最关键的窗口。
4. 盲区区域：当前视角落在两个孔接受区之间，没有任何孔可透光。它不同于“轴上退化为单孔”。
