...
| metre (m) | 米 | 国际单位制中的长度单位，用于屈光度、波长和孔径计算。 |
| millimetre (mm) | 毫米 | 长度单位，`1 mm = 0.001 m`。本阶段用于表示孔径。 |
| nanometre (nm) | 纳米 | 长度单位，`1 nm = 10⁻⁹ m`。本阶段用于表示光的波长。 |
...
5 分记录法中的 `5.0`、`4.8`、`4.6` 等数值表示视力表读数。它和十进制小数视力满足：
...
| 近视 M | 计算式 | 理论 `d*` |
|---:|---|---:|
| `1 D` | `√(1.342 × 10⁻⁶ / 1)` | `1.158 mm` |
| `2 D` | `√(1.342 × 10⁻⁶ / 2)` | `0.819 mm` |
| `3 D` | `√(1.342 × 10⁻⁶ / 3)` | `0.669 mm` |
| `4 D` | `√(1.342 × 10⁻⁶ / 4)` | `0.579 mm` |
| `6 D` | `√(1.342 × 10⁻⁶ / 6)` | `0.473 mm` |
...
4. 为后续多孔阵列、透过率和视网膜图像卷积提供设计初值。

### 7. 本阶段结论与后续工作

550 nm 结果表明，近视程度越高，最优针孔直径越小。近视度数从 `1 D` 增加到 `6 D` 时，仿真最优孔径从 `1.1167 mm` 减小到 `0.4559 mm`。由于理论公式包含 `1 / √M`，孔径的下降速度低于近视度数的增加速度。

当前验证包括圆孔第一暗环、五个最优孔径、PSF 能量归一化和离焦符号对称性，均通过设定误差范围。

后续工作包括：

1. 使用扩展太阳光源计算多孔阵列的重影位置和亮度。
2. 把 PSF 与 E 字视标或兰多尔特 C 视标卷积，生成视网膜图像。
3. 计算针孔阵列的透过率、照度和不同设计之间的综合差异。
4. 增加网格收敛性和几何极限验证，确认结果不依赖计算网格。
